...
$(document).ready(function () {
  ...
  // Function to make the POST request for places search
  function fetchPlaces() {
    const amenitiesList = Object.keys(selectedAmenities); // Get selected amenities' IDs

    // Send POST request with the selected amenities
    $.ajax({
      url: 'http://0.0.0.0:5001/api/v1/places_search',  // Correct URL for the places_search endpoint
      method: 'POST',
      contentType: 'application/json',
      data: JSON.stringify({ amenities: amenitiesList }), // Send selected amenities in the request body
  ...
  // Trigger fetchPlaces when the button is clicked
